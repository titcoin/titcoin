...
BOOST_AUTO_TEST_CASE(coin_selection_tests)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        BOOST_CHECK_EQUAL(nValueRet, 1 * COIN);  // we should get 1 TIT in 1 coin
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);

        BOOST_CHECK( testWallet.SelectCoinsMinConf(195 * CENT, 1, 1, 0, vCoins, setCoinsRet, nValueRet));
        BOOST_CHECK_EQUAL(nValueRet, 2 * COIN);  // we should get 2 TIT in 1 coin
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);
        ...
        // test randomness
        {
            ...
            BOOST_CHECK(testWallet.SelectCoinsMinConf(69 * COIN, 1, 6, 0, vCoins, setCoinsRet , nValueRet));
            BOOST_CHECK(testWallet.SelectCoinsMinConf(69 * COIN, 1, 6, 0, vCoins, setCoinsRet2, nValueRet));
...
BOOST_FIXTURE_TEST_CASE(rescan, TestChain100Setup)
{
    ...
    // and new block files.
    {
        ...
        BOOST_CHECK_EQUAL(wallet.GetImmatureBalance(), 138 * COIN);
    }
    ...
    // file.
    {
        ...
        BOOST_CHECK_EQUAL(wallet.GetImmatureBalance(), 69 * COIN);
    }
    ...
    // after.
    {
        ...
        BOOST_CHECK_EQUAL(response.write(),
            strprintf("[{\"success\":false,\"error\":{\"code\":-1,\"message\":\"Rescan failed for key with creation "
                      ...
                      "by pruning or data corruption (see titcoind log for details) and could be dealt with by "
...
BOOST_FIXTURE_TEST_CASE(coin_mark_dirty_immature_credit, TestChain100Setup)
{
    ...
    wtx.MarkDirty();
    wallet.AddKeyPubKey(coinbaseKey, coinbaseKey.GetPubKey());
    BOOST_CHECK_EQUAL(wtx.GetImmatureCredit(), 69*COIN);
}
...
BOOST_FIXTURE_TEST_CASE(ListCoins, ListCoinsTestingSetup)
{
    ...
    BOOST_CHECK_EQUAL(69 * COIN, wallet->GetAvailableBalance());
