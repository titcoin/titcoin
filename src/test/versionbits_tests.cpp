...
BOOST_AUTO_TEST_CASE(versionbits_computeblockversion)
{
    ...
    const auto chainParams = CreateChainParams(CBaseChainParams::MAIN);
    const Consensus::Params &mainnetParams = chainParams->GetConsensus();
    int nMinerConfirmationWindow = mainnetParams.nMinerConfirmationWindow;
    int64_t nPowTargetSpacing = mainnetParams.nPowTargetSpacing;
    ...
    lastBlock = firstChain.Mine(nMinerConfirmationWindow, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);

    // Mine (nMinerConfirmationWindow-5) more blocks at the old time, and check that CBV isn't setting the bit yet.
    for (int i=1; i<(nMinerConfirmationWindow-4); i++) {
        lastBlock = firstChain.Mine(nMinerConfirmationWindow+i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    ...
    nTime = nStartTime;
    for (int i=(nMinerConfirmationWindow-4); i<=nMinerConfirmationWindow; i++) {
        lastBlock = firstChain.Mine(nMinerConfirmationWindow+i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);
    }

    // Advance to the next period and transition to STARTED,
    lastBlock = firstChain.Mine(nMinerConfirmationWindow*3, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    ...
    nTime += nPowTargetSpacing;
    int blocksToMine = nMinerConfirmationWindow*2; // test blocks for up to 2 time periods
    int nHeight = nMinerConfirmationWindow*3;
    ...
    for (int i=0; i<(nMinerConfirmationWindow-1); i++) {
        lastBlock = firstChain.Mine(nHeight+1, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);
        nHeight += 1;
    }
    ...
    // next period.
    lastBlock = secondChain.Mine(nMinerConfirmationWindow, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);

    // Mine another period worth of blocks, signaling the new bit.
    lastBlock = secondChain.Mine(nMinerConfirmationWindow*2, nTime, VERSIONBITS_TOP_BITS | (1<<bit)).Tip();
    ...
    lastBlock = secondChain.Mine((nMinerConfirmationWindow*3 - 1), nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);
    lastBlock = secondChain.Mine(nMinerConfirmationWindow*3, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);
